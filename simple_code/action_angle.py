...
# --------------- Save results ----------------
    ...
    if is_phase_space and poincare_mode != "none":
        ...
        file_path = os.path.join(output_dir, f"cartesian_a{a_str}_nu{float(omega_str)/par.omega_s:.2f}.npz")
        ...
        output_dir,
        f"cartesian_a{a_start_str}-{a_end_str}_nu{float(omega_start_str)/par.omega_s:.2f}-{float(omega_end_str)/par.omega_s:.2f}.npz"
        )

    elif poincare_mode == "none":
        file_path = os.path.join(output_dir, "cartesian_full_steps.npz")
